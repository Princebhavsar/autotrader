...
function photography_pagination($pages = '', $range = 4)
{
	 ...
	 if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."'>&laquo; ".esc_html_e('First', 'photography-translation')."</a>";
	 if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."'>&lsaquo; ".esc_html_e('Previous', 'photography-translation')."</a>";
	 ...
	 if ($paged < $pages && $showitems < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\">".esc_html_e('Next', 'photography-translation')." &rsaquo;</a>";
	 if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."'>".esc_html_e('Last', 'photography-translation')." &raquo;</a>";
	 echo "</div>";
	 }
}
...
function photography_import_data( $data ) 
{
	...
	if ( empty( $data ) || ! is_object( $data ) ) {
		wp_die(
			esc_html__('Import data could not be read. Please try a different file.', 'photography-translation' ),
			'',
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
			...
			$sidebar_message = esc_html__('Sidebar does not exist in theme (using Inactive)', 'photography-translation' );
		}
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__('Site does not support widget', 'photography-translation' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__('Widget already exists', 'photography-translation' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__('Imported', 'photography-translation' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__('Imported to Inactive', 'photography-translation' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__('No Title', 'photography-translation' ); // show "No Title" if widget instance is untitled
...
function photography_live_builder_begin_wrapper($item_id = '', $item_type = '')
{
	...
	if(isset($_GET['ppb_live']) && !empty($item_id))
	{
		...
		{
			$return_html.= '<a href="javascript:;" class="ppb_add_after" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Add', 'photography-translation' ).'"><span class="dashicons dashicons-arrow-down-alt"></span></a>';
		
			$return_html.= '<a href="javascript:;" class="ppb_edit" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Edit', 'photography-translation' ).'"><span class="dashicons dashicons-welcome-write-blog"></span></a>';
			
			$return_html.= '<a href="javascript:;" class="ppb_duplicate" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Duplicate', 'photography-translation' ).'"><span class="dashicons dashicons-admin-page"></span></a>';
		}
		
		$return_html.= '<a href="javascript:;" class="ppb_remove" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Remove', 'photography-translation' ).'"><span class="dashicons dashicons-no"></span></a>';
...
function photography_get_hover_effect($type = 'gallery')
{
	if($type == 'gallery')
	{
		$tg_gallery_hover_effect = kirki_get_option('tg_gallery_hover_effect');
		
		return $tg_gallery_hover_effect;
	}
	else if($type == 'portfolio')
	{
		$tg_portfolio_hover_effect = kirki_get_option('tg_portfolio_hover_effect');
...
function photography_get_lightbox_caption_attr($photo_id = '', $with_purchase = true, $poster = '')
{
	...
	if(!empty($tg_lightbox_enable) && !empty($photo_id) && !empty($tg_lightbox_enable_caption)) 
	{
		...
		{
			$purchase_html = '<a href="'.esc_url($photography_purchase_url).'" class="button ghost"><i class="fa fa-shopping-cart marginright"></i>'.esc_html__('Purchase', 'photography-translation' ).'</a>';
		}
...
/*function photography_get_progressive_preview_image_name($image_size = '')
{
	$return_image_size = '';
	...
	if(!empty($tg_enable_lazy_loading))
	{
		switch($image_size)
		{
			case 'photography-gallery-masonry':
				$return_image_size = 'photography-gallery-masonry';
			break;
		}
	}
	else
